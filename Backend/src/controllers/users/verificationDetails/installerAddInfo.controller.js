...
const installerAddInfo = asyncHandler(async (req, resp)=>{
    const user = req.user;
    if(!user){
    ...
    const updatedUser = await Installer.findOneAndUpdate(
        {
            _id: user._id
        },
        {
    ...
    const finalUser = await Installer.findById(updatedUser._id).select("-Password -refreshToken");

    resp.status(200).json(new ApiResponse(200 , {User: finalUser}, "User Updated Successfuly"));
});

module.exports = installerAddInfo;