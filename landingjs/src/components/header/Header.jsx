  ...
  return (
    <section id="header-section" className={`header-section ${openSearch ? '-brite' : ''}`}>
      <div className="header-section-logo">
        <Image
          src={openSearch ? logoLite : scrolled ? logoDark : logoLite}
          alt="wings lens logo"
          className="header-section-logo-img"
          height={70}
        />
      </div>
      ...
      <div className="header-section-options">
        {windowWidth < 1150 ? (
          <div
            className="header-section-search-div"
          ...
          <>
            <div
              className="header-section-options-div resp-none"
            ...
            >
              ...
              <div className="header-section-options-item">
                <p className={`header-section-options-item-p ${scrolled ? 'txt-black' : ''}`}>
                  <FiSearch size={20} style={{ cursor: 'pointer' }} onClick={() => setOpenSearch(true)} />
                </p>
              </div>
            </div>
      ...
      <div className="header-section-search-resp -resp">
        <p className={`header-section-options-item-p ${scrolled ? 'txt-black' : ''}`}>
          <FiSearch
            size={20}
            style={{ display: openSearch ? 'none' : 'flex', cursor: 'pointer' }}
            onClick={() => setOpenSearch(true)}
          />
        </p>
        <p className={`header-section-options-item-p ${scrolled ? 'txt-black' : ''}`}>
          <MdMenu size={30} onClick={() => setOpenMenu(true)} />
        </p>
      </div>
    </section>
  );
